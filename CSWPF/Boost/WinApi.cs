...
namespace CSWPF.Boost
{
    public static class WinApi
    {
        ...
        {
            int num1 = (int)WinApi.VkKeyScan(ch);
            Keys virtualKey = (Keys)(num1 & (int)byte.MaxValue);
            int num2 = num1 >> 8;
            if ((num2 & 1) != 0)
                virtualKey |= Keys.Shift;
            if ((num2 & 2) != 0)
                virtualKey |= Keys.Control;
            if ((num2 & 4) != 0)
                virtualKey |= Keys.Alt;
            return virtualKey;
        }

        public static void SendCtrlHotKey(IntPtr hwnd)
        {
            foreach (char ch in System.Windows.Clipboard.GetText())
            {
                Keys virtualKey = WinApi.ConvertCharToVirtualKey(ch);
                uint lParam = (uint)(1 | (int)WinApi.MapVirtualKey((uint)virtualKey, 0U) << 16);
                WinApi.PostMessage(hwnd, 256U, (IntPtr)(int)virtualKey, (IntPtr)(long)lParam);
            }
        }
