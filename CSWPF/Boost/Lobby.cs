...
namespace CSWPF.Boost
{
    ...
    public class Lobby
    {
            ...
            for (index = 0; index < accounts.Count; ++index)
            {
                ...
                if (string.IsNullOrEmpty(str))
                {
                    ...
                    if (process != null)
                    {
                        ...
                        if (bot.SharedSecret != str)
                        {
                            bot.SharedSecret = str;
                            LobbyDb.G.Save();
                        }
                    }
                    else
                    ...
                    Log.Write("Acc: " + bot.Login + " had code " + bot.SharedSecret);
                dict.Add(bot, str);
                if (index != max)
                    bot = (Account)null;
                else
        ...
        public void TaskGameState(GameState gs, Lobby lobby)
        {
            ...
            Account key = this.Accounts.FirstOrDefault<Account>((Func<Account, bool>)(o => !string.IsNullOrEmpty(Convert.ToString(o.SteamID)) && o.SteamID.Equals(streamId)));
            ...
            if (string.IsNullOrEmpty(streamId) || !streamId.Equals(this.Accounts[0].SteamID) && !streamId.Equals(this.Accounts[5].SteamID))
                return;
            this._mapMode = gs.Map.Mode;
            this._mapPhase = gs.Map.Phase;
            if (this._winLeader != null && this._winLeader.SteamID.Equals(gs.Player.SteamID))
            {
            ...
            if (this._loseLeader == null || !this._loseLeader.SteamID.Equals(gs.Player.SteamID))
                return;
        ...
        {
            User[] accs = accounts.Select<Account, User>((Func<Account, User>)(o => o.GetData())).ToArray<User>();  
            int num = await Task.Run<int>((Func<int>)(() => CPPBoosterImport.Connect(accs, accs.Length, this.Width, this.Height, (IsCancellationRequested)(() => token.IsCancellationRequested), (GetMainWindowHandle)(acc => this.GetWindowHandle(acc)))));
        }

        public IntPtr GetWindowHandle(User acc)
        {
        ...
        public async Task TaskCycles(CancellationToken token)
        {
            ...
            try
            {
                ...
                Log.Write(lobbyName + " - Started cycles!!!");
                ...
                foreach (Account account in team)
                    Log.Write(lobbyName + " - " + account.Login + " " + account.SteamID);
                label_96:
        ...
        private void RecalcPositions(Account account, int x, int y, out int clickX, out int clickY)
        {
            ...
            int num1 = (Settings.W - account.Lobby.Width) / 2;
            int num2 = Settings.H - account.Lobby.Height - num1;
        ...
        private void MoveToAccount(Account account, int x, int y, out int clickX, out int clickY)
        {
            ...
            Log.Write(string.Format("Move to {0} Pos={1},{2},{3},{4} Sz={5},{6}", (object)account.Login, (object)x, (object)y, (object)Settings.W, (object)Settings.H, (object)account.Lobby.Width, (object)account.Lobby.Height));
        }
