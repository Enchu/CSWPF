...
public class AccountHelper
  {
    ...
    public static List<Sandbox> Storage { get; set; } = new List<Sandbox>();

    public static string GetFreeBox(Account account)
    {
      lock (AccountHelper.Storage)
      {
        Sandbox sandbox = AccountHelper.Storage.FirstOrDefault<Sandbox>((Func<Sandbox, bool>) (o => o.Account == account));
        if (sandbox != null)
          return sandbox.BoxName;
        foreach (string enumBox in Sandboxie.G.EnumBoxes())
        {
          string sb = enumBox;
          if (!AccountHelper.Storage.Any<Sandbox>((Func<Sandbox, bool>) (o => o.BoxName.Equals(sb))))
            return sb;
        }
      }
      throw new CantFindFreeSandboxException();
    }

    public static Sandbox GetSandbox(string boxName)
    {
      lock (AccountHelper.Storage)
        return AccountHelper.Storage.FirstOrDefault<Sandbox>((Func<Sandbox, bool>) (o => o.BoxName.Equals(boxName)));
    }

    public static Sandbox GetSandbox(Account account)
    {
      lock (AccountHelper.Storage)
        return AccountHelper.Storage.FirstOrDefault<Sandbox>((Func<Sandbox, bool>) (o => o.Account == account));
    }

    public static void RemoveSandbox(Account account)
    {
      lock (AccountHelper.Storage)
      {
        Sandbox sandbox = AccountHelper.Storage.FirstOrDefault<Sandbox>((Func<Sandbox, bool>) (o => o.Account == account));
        if (sandbox == null)
          return;
        AccountHelper.Storage.Remove(sandbox);
      }
    }

    public static Sandbox AddSandbox(string boxName, Account account)
    {
      lock (AccountHelper.Storage)
      {
        Sandbox sandbox1 = AccountHelper.GetSandbox(boxName);
        if (sandbox1 != null)
        {
          sandbox1.Account = account;
          return sandbox1;
        }
        ...
        AccountHelper.Storage.Add(sandbox2);
        return sandbox2;
      }
    }
  }